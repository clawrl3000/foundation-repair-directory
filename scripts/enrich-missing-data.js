...
/**
 * Foundation Scout Data Quality Cleanup
 * 
 ...
 * 3. If no match is found, mark it by setting a flag for removal
 * 4. After all enrichment, deactivate unmatched businesses (set is_active=false)
 * 
 * Usage: node scripts/enrich-missing-data.js
 */
...
function isGoodMatch(businessName, googleResult) {
  ...
  const foundationTerms = ['foundation', 'basement', 'structural', 'concrete'];
  ...
  // Must have at least one foundation-related term
  const hasFoundationTerm = foundationTerms.some(term => 
    googleLower.includes(term) || businessLower.includes(term)
  );
  ...
  const uniqueBusinessWords = businessWords.filter(word => !['foundation', 'repair', 'solutions', 'systems', 'services'].includes(word));
...
async function updateBusinessWithEnrichment(businessId, placeDetails, confidence) {
  ...
  if (placeDetails.user_ratings_total) {
    updateData.google_review_count = placeDetails.user_ratings_total;
  }
...
/**
 * Mark business as unmatched for potential removal
 */
async function markBusinessAsUnmatched(businessId) {
  const updateData = {
    enrichment_source: 'attempted_google_places',
    enrichment_confidence: 'no_match',
    enrichment_date: new Date().toISOString(),
    enrichment_notes: 'No matching business found in Google Places - likely placeholder'
  };
...
async function deactivateUnmatchedBusinesses() {
  console.log('\n🗑️ Deactivating unmatched placeholder businesses...');
  
  const { data: unmatchedBusinesses, error: fetchError } = await supabase
    .from('businesses')
    .select('id, name')
    .eq('enrichment_confidence', 'no_match')
    .eq('is_active', true);
  
  if (fetchError) {
    console.error('❌ Error fetching unmatched businesses:', fetchError.message);
    return;
  }
  
  if (!unmatchedBusinesses || unmatchedBusinesses.length === 0) {
    console.log('ℹ️ No unmatched businesses to deactivate');
    return;
  }
  
  console.log(`📋 Found ${unmatchedBusinesses.length} unmatched businesses to deactivate`);
  ...
  const { error: updateError } = await supabase
    ...
    .update({ 
      ...
      deactivation_reason: 'No real business found - placeholder data'
    })
    .eq('enrichment_confidence', 'no_match')
    .eq('is_active', true);
  
  if (updateError) {
    console.error('❌ Error deactivating businesses:', updateError.message);
    return;
  }
  
  stats.deactivated = unmatchedBusinesses.length;
  console.log(`✅ Deactivated ${unmatchedBusinesses.length} placeholder businesses`);
}
  ...
  try {
    ...
    for (const business of businesses) {
      ...
      if (googleResults.length === 0) {
        ...
        await markBusinessAsUnmatched(business.id);
